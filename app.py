...
cluster = MongoClient("mongodb+srv://Kelvin:Kelvin@whatsapp.aagnrkw.mongodb.net/?retryWrites=true&w=majority&appName=Whatsapp", tls=True, tlsAllowInvalidCertificates=True)
...
def reply():
    ...
    profile_name = request.form.get("ProfileName", "Unknown User")
    ...
    if bool(user) == False:
        ...
        if option == 0:
            ...
            cakes = ["Red Velvet Cake", "Dark Forest Cake", "Ice Cream Cake",
                     "Plum Cake", "Sponge Cake", "Genoise Cake", "Angel Cake", "Carrot Cake"]
            ...
            res.message("please enter your address to confirm your order")

        else:
            res.message("please kindly enter a valid response")
        ...
        orders.insert_one({"number": number, "item": selected, "address": text, "Order Time": datetime.now()})
